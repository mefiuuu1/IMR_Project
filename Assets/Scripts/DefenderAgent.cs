...
public class DefenderAgent : Agent
{
    ...
    [Header("Mode")]
    public bool useNeuralNetwork = false;  // FALSE = direct reactive, TRUE = use model
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if (playerSword != null && playerSword.attackInProgress && !attackProcessed)
        {
            ...
            // MODUL REACTIV: blocheaza imediat in directia corecta
        ...
        // Reset cand atacul se termina
        if (playerSword != null && !playerSword.attackInProgress)
        {
            attackProcessed = false;
        }

        // MODUL NEURAL: foloseste output-ul modelului
        if (useNeuralNetwork)
        {
